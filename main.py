...
async def get_tables_status(restaurant_id: str):
    conn = get_db()
    cursor = conn.cursor()
...
async def get_current_shift(restaurant_id: str):
    conn = get_db()
    cursor = conn.cursor()
...
async def open_shift(restaurant_id: str, shift: ShiftOpen):
    conn = get_db()
    cursor = conn.cursor()
...
async def close_shift(restaurant_id: str, shift_id: str, shift: ShiftClose):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('''
...
async def get_dashboard(restaurant_id: str):
    conn = get_db()
    cursor = conn.cursor()
    ...
    return {
        "success": True,
        "today_earnings": today_stats["total_earnings"] or 0,
